...
bool EpubReaderActivity::pageTurnImpl(bool isForwardTurn) {
  ...
  if (isForwardTurn) {
    if (section->currentPage < section->pageCount - 1 || section->isBuilding()) {
      section->currentPage++;
      // Keep the resume hint on the page actually being read: if the section is dropped
      // later (a failed background build tick, a settings reflow), renderBook() rebuilds
      // from nextPageNumber, and a stale one restarts the chapter from the top.
      nextPageNumber = section->currentPage;
      lastPageTurnTime = millis();
      return true;
    ...
    if (section->currentPage > 0) {
      section->currentPage--;
      nextPageNumber = section->currentPage;  // see the forward branch
      lastPageTurnTime = millis();
      return true;
...
void EpubReaderActivity::renderStatusBar() const {
  ...
  if (automaticPageTurnActive) {
    ...
    title = epub ? utf8ComposeNfc(epub->getTitle()) : "";  // NFC so NFD titles render correctly
  }
...
bool EpubReaderActivity::usesToolbarMenu() const {
  // Aurora owns the reader chrome, so it always gets the toolbar. Other themes
  // opt in through the setting, but only on touch boards: button boards keep
  // the classic list menu even if a settings file (an SD card moved from a
  // touch board) says Toolbar.
  return GUI.ownsReaderChrome() ||
         (mappedInput.hasTouch() && SETTINGS.readerMenuStyle == CrossPointSettings::READER_MENU_TOOLBAR);
}
  ...
  const auto activateRow = [this, count, &fastRedraw] {
    ...
    if (overlay == Overlay::Text) {
      if (panelIndex == 0) {
        ...
        discardOverlayPage();
        // Aurora's own font picker (built-in + SD fonts), the same screen Settings uses.
