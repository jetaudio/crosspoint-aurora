...
void DropCapFontSelectionActivity::onEnter() {
  ...
  if (registry_) {
    for (const auto& family : registry_->getFamilies()) {
      entries_.push_back({family.name, false});
    }
  }
...
void DropCapFontSelectionActivity::loop() {
  // Back/Confirm on release (not press) to match the settings UI convention: the
  // parent SettingsActivity navigates on release, so consuming the press here would
  // let the release leak through and bounce it to the Library.
