...
class UiListActivity : public Activity, protected UiAppHost {
 ...
 protected:
  ...
  // Sub-screens return backHeader(parent) here ("‹ Parent", see Activity).
  virtual const char* headerTitle() const { return nullptr; }
  // A "‹ Parent" title makes the header a touch Back target.
  bool hasTouchBackHeader() const override;
