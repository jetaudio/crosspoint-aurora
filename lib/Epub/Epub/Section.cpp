...
namespace {
// v27 (upstream baseline): words NFC-composed at layout time so NFD Vietnamese
// chapter headings/body render with precomposed glyphs; invalidates NFD caches.
// Aurora drop-cap additions layered on top (each adds/changes header state, so the
// version is bumped past upstream to invalidate all older section caches):
//   - dropCapsEnabled flag in the header (re-paginates when toggled)
