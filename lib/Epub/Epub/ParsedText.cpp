...
static bool isLeadingCapPunct(const uint32_t cp) {
  switch (cp) {
...
bool ParsedText::buildDropCapPrefix(std::string& outText, uint32_t& letterCp, EpdFontFamily::Style& letterStyle) const {
  ...
  for (size_t wi = 0; wi < words.size(); ++wi) {
    ...
    const auto* p = reinterpret_cast<const unsigned char*>(wordStore.cstr(words[wi]));
    uint32_t cp;
    while ((cp = utf8NextCodepoint(&p)) != 0) {
      ...
      // it is actually an alphabetic initial.
      utf8AppendCodepoint(cp, outText);
      letterCp = cp;
...
void ParsedText::stripLeadingCodepoints(size_t count) {
  while (count > 0 && !words.empty()) {
    const auto* base = reinterpret_cast<const unsigned char*>(wordStore.cstr(words[0]));
    const auto* p = base;
    while (count > 0 && *p != 0) {
      utf8NextCodepoint(&p);
      --count;
    }
    const size_t consumed = static_cast<size_t>(p - base);
    if (consumed < words[0].len) {
      // The suffix shares the word's arena bytes and inherits its release.
      words[0] = WordStore::suffix(words[0], consumed);
    } else {
      // Whole leading word consumed by the cap prefix: drop it from every parallel array.
      wordStore.release(words[0]);
...
std::vector<size_t> ParsedText::computeDropCapLineBreaks(const GfxRenderer& renderer, const int fontId,
  ...
  while (currentIndex < wordWidths.size()) {
    ...
    while (currentIndex < wordWidths.size()) {
      ...
      } else if (!isFirstWord && wordNoSpaceBefore[currentIndex]) {
      ...
      if (smallCapsFirstLine_ && lineOrdinal == 0) {
        wordW = measureWordWidth(renderer, fontId, smallCapsUppercase(std::string(wordAt(currentIndex))),
                                 wordStyles[currentIndex], blockStyle.characterSpacing, wordSpacingPercent);
      }
...
void ParsedText::layoutAndExtractLines(const GfxRenderer& renderer, const int fontId, const uint16_t viewportWidth,
                                       ...
                                       const bool smallCapsFirstLine, const int8_t characterSpacing,
  ...
  // around the cap. A spec with empty text is a wrap-only request (inset only, no glyph).
  dropCap_ = dropCap;
  std::string capText;
  if (dropCap_) {
  ...
  if (smallCapsFirstLine_ && lineCount > 0) {
    ...
    for (size_t i = 0; i < firstLineEnd; ++i) {
      const std::string upper = smallCapsUppercase(std::string(wordAt(i)));
      WordStore::StoredWord stored;
      if (!storeWord(upper, stored)) continue;  // OOM: keep the lower-case word
      wordStore.release(words[i]);
      words[i] = stored;
      wordWidths[i] =
          measureWordWidth(renderer, fontId, wordAt(i), wordStyles[i], blockStyle.characterSpacing, wordSpacingPercent);
    }
  }
  ...
  for (size_t i = 0; i < lineCount; ++i) {
    if (dropCap_ && i == 0 && !capText.empty()) {
      ...
      const DropCapSpec* dc = dropCap_;
      auto capProcess = [&](std::unique_ptr<TextBlock> block, uint32_t offset) {
        if (block) block->setDropcap(capText, dc->style, dc->scale);
        processLine(std::move(block), offset);
      };
      extractLine(i, pageWidth, wordWidths, wordContinues, wordNoSpaceBefore, lineBreakIndices, capProcess, renderer,
                  fontId);
    } else {
...
void ParsedText::extractLine(const size_t breakIndex, const int pageWidth, const std::vector<uint16_t>& wordWidths,
  ...
  // For drop-cap paragraphs this returns the wrap inset for the first `lineSpan` lines,
  // shifting the line's words right of the cap; otherwise it's the normal first-line indent.
